...
var  LadyBossBot = require('../src/LadyBossBot');
...
describe('Bot Initialization', function () {
  ...
  beforeEach(function () {
    ...
    this.ladyBossBot = new LadyBossBot('Fake-token-slack');
  ...
  it('should the BOT token present', function () {
    expect(this.ladyBossBot.bot.token).to.be.equal('Fake-token-slack');
  });

  it('should the BOT say hello to the the channel when start', function () {
    this.ladyBossBot.bot.emit('start');

    expect(this.textCheck).to.be.equal('Hello I am lady-boss-bot');
  });
});
